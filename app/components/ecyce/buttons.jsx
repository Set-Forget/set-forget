...
const Button = ({ type, text, className }) => {
  ...
  if (type === "solid") {
    return (
      <button 
      className={`relative bg-sf-lime text-sf-dblue flex justify-center items-center rounded-3xl hover:shadow-xl hover:shadow-sf-lime/30 transition-all duration-500 w-64 h-[74px] mt-8 lg:mt-12 ${className || ""}`}
      onMouseEnter={() => setHover(true)}
      onMouseLeave={() => setHover(false)}
      >
        <p className="text-2xl font-bold">{text}</p>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 8" fill="none" className={`size-6 ${hover ? 'ml-8' : 'ml-3'} transition-all delay-150 origin-center duration-200 ease-in-out`}>
